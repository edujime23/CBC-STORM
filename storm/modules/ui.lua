...
-- Controller UI: military CMD-style, cursor-safe, hotkeys never leak.
-- Views: Home, Pairing wizard, Approvals, Workers (live refresh with ping/status/fire/log).
...
local function header()
  ...
  setc(COL.headerBG, COL.headerFG)
  term.clearLine()
  term.write(line)
  setc()
end
...
local function footer()
  ...
  setc(COL.bg, COL.footerFG)
  safe_write_line(h, "[P] Pair  [A] Approvals  [W] Workers  [Q] Quit", COL.footerFG)
  setc()
end
...
-- Flush pending char events so hotkeys don't leak into prompts
...
-- Read line from bottom prompt using only char/key (does not consume modem_message)
...
local function redraw_home()
  ...
  safe_write_line(4, "Use keys: [P] Pair  [A] Approvals  [W] Workers  [Q] Quit")
  footer()
  pairing_info()
end
...
local function approvals_screen()
  ...
  while true do
    ...
    local function allowed(c) return (c>='0' and c<='9') or c=='d' or c=='D' or c=='q' or c=='Q' end
    local inp = read_line_filtered("", allowed)
...
-- Workers screen: live refresh + input; input only listens to char/key (won't consume modem_message)
local function workers_screen()
    ...
    for i = 1, list_h do
      ...
      if it then
        ...
        term.write(("[%d] dev=%s  modem=%s  rtt=%s  status=%s"):format(
          i, tostring(it.dev_id), tostring(it.modem or "?"), rtt, st))
      end
    end
  end
  ...
  local function refresh_loop()
    while running do
      ...
      footer()              -- re-draw footer here
      pairing_info()        -- do not suspend bottom line permanently
      prompt_draw()
      U.sleep_ms(400)
    end
  end
    ...
    while running do
      ...
      elseif got_key then
        if key == keys.backspace then
          if #input_buf > 0 then input_buf = input_buf:sub(1, #input_buf-1); prompt_draw() end
        elseif key == keys.enter then
          ...
          elseif cmd:match("^[pP]%d+$") then
            local idx = tonumber(cmd:sub(2)); local it = list[idx]
            if it then
              local ok, err = Join.ping_worker(it.dev_id)
              status_msg = ok and ("Ping sent to dev="..tostring(it.dev_id)..". Waiting for PONG...") or ("Ping failed: "..tostring(err))
          ...
          elseif cmd:match("^[lL]%d+$") then
            local idx = tonumber(cmd:sub(2)); local it = list[idx]
            if it then
              safe_write_line(status_y, "Enter log text (ESC to cancel):", COL.statusFG)
